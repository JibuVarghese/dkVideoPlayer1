...
public class GestureView extends FrameLayout implements IGestureComponent {
    ...
    private LinearLayout mCenterContainer,mDoubleTapContainer,mllRewind,mllForward;
...

    {
        ...
        mDoubleTapContainer = findViewById(R.id.doubleTapContainer);
        mllRewind = findViewById(R.id.llRewind);
        mllForward = findViewById(R.id.llForward);
    }
    ...
    public void onStopSlide() {
        ...
        if (mDoubleTapContainer.getVisibility() == VISIBLE){
            mDoubleTapContainer.animate()
                    .alpha(0f)
                    .setDuration(600)
                    .setListener(new AnimatorListenerAdapter() {
                        @Override
                        public void onAnimationEnd(Animator animation) {
                            super.onAnimationEnd(animation);
                            mDoubleTapContainer.setVisibility(GONE);
                        }
                    })
                    .start();
        }
    ...
    @Override
    public void doubleTapForward() {
        mDoubleTapContainer.setVisibility(VISIBLE);
        mDoubleTapContainer.setAlpha(1f);
        mllRewind.setVisibility(INVISIBLE);
        mllForward.setVisibility(VISIBLE);
        mllForward.performClick();
    }

    @Override
    public void doubleTapRewind() {
        mDoubleTapContainer.setVisibility(VISIBLE);
        mDoubleTapContainer.setAlpha(1f);
        mllForward.setVisibility(INVISIBLE);
        mllRewind.setVisibility(VISIBLE);
        mllRewind.performClick();
    }
